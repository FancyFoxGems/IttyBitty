...
#pragma region [Database] DEFINITION
	...
	public:
		...
		VIRTUAL CDBRESULT CreateTable(CSIZE, PCCHAR = NULL, CSIZE = DB_DEFAULT_INITIAL_ROWS_ALLOCATION, CBYTE = MAX_OF(BYTE));
		...
		VIRTUAL CDBRESULT DeleteFrom(CBYTE, CSIZE, CBOOL = TRUE);
		VIRTUAL CDBRESULT DeleteFrom(PCCHAR, CSIZE, CBOOL = TRUE);

		VIRTUAL CDBRESULT TruncateTable(CBYTE, CBOOL = TRUE);
		VIRTUAL CDBRESULT TruncateTable(PCCHAR, CBOOL = TRUE);
	...
	protected:
		...
		VIRTUAL CDBRESULT MoveTables(RCDWORD, RCLONG, CBOOL = TRUE);
